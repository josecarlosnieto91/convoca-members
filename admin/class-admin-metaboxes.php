...
class Admin_Metaboxes
{
    public function __construct()
    {
        ...
        add_action('save_post_miembro', [$this, 'save_metaboxes']);

        // AJAX Handlers
        add_action('wp_ajax_bdv_send_payment_link', [$this, 'ajax_send_payment_link']);
        add_action('wp_ajax_bdv_send_reminder', [$this, 'ajax_send_reminder']);
        add_action('wp_ajax_bdv_export_member_data', [$this, 'ajax_export_member_data']);
        add_action('wp_ajax_bdv_delete_member_data', [$this, 'ajax_delete_member_data']);

        // User Profile (for Volunteers)
        add_action('show_user_profile', [$this, 'render_user_documents_section'], 10, 1);
        add_action('edit_user_profile', [$this, 'render_user_documents_section'], 10, 1);
    }
    ...
    public function ajax_send_payment_link(): void
    {
        ...
        if (!$plan) {
            wp_send_json_error(['message' => 'Plan no válido o no asignado.']);
        }

        $amount = $plan['price'];
        if ($amount <= 0) {
            wp_send_json_error(['message' => 'Este plan no tiene coste económico.']);
        }
        ...
        if (!\Convoca\Core\Features::is_gateway_active() || !function_exists('Convoca\Gateway\bdv_gateway_create_payment')) {
            wp_send_json_error(['message' => 'Gateway no activo.']);
        }
        ...
        $email_manager->send_recordatorio_pago($post_id, ['{link_pago}' => $result['payment_url']]);

        wp_send_json_success(['message' => 'Enlace generado y enviado correctamente. Recargando...', 'reload' => true]);
    }
    ...
    public function ajax_send_reminder(): void
    {
        ...
        if (!$pago_id) {
            wp_send_json_error(['message' => 'No hay un pago activo. Genera uno primero.']);
        }
        ...
        $email_manager = new Email_Manager();
        $email_manager->send_recordatorio_pago($post_id, ['{link_pago}' => $payment_url]);

        wp_send_json_success(['message' => 'Recordatorio enviado correctamente.']);
    }
    ...
    private function verify_ajax(): void
    {
        ...
        if (!current_user_can('edit_post', $post_id)) {
            wp_send_json_error(['message' => 'Sin permisos sobre este registro.']);
        }
    ...
    public function ajax_delete_member_data(): void
    {
        ...
        wp_send_json_success(['message' => 'Todos los datos del miembro han sido eliminados.']);
    }
}

...
class Admin_Import_CSV
{
    public function __construct()
    {
        ...
        add_action('admin_post_bdv_import_csv_preview', [$this, 'handle_preview']);
        add_action('admin_post_bdv_import_csv_run', [$this, 'handle_import']);
        add_action('admin_post_bdv_import_csv_template', [$this, 'handle_template_download']);
        add_action('admin_post_bdv_import_csv_continue', [$this, 'handle_batch_continue']);
    }
    ...
    public function render(): void
    {
        ...
        <div class="wrap" style="max-width:900px;">
            ...
            <?php if ($batch_progress): ?>
                <div class="biodevas-box" style="background:#fff;border-radius:12px;padding:30px;margin-top:20px;">
                    ...
                    <p style="font-size:1.2em;">
                        <?php echo sprintf(
                            __('Procesados %d de %d registros...', 'convoca-members'),
            ...
            <?php if ($preview): ?>
                <div class="biodevas-box" style="background:#fff;border-radius:12px;padding:30px;margin-top:20px;">
                    <h2><?php echo sprintf(__('2. Vista previa (%d filas detectadas)', 'convoca-members'), $preview['total']); ?></h2>
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        ...
                        <div class="biodevas-field">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                                <?php foreach (['nombre' => 'Nombre', 'email' => 'Email', 'dni' => 'DNI', 'plan' => 'Plan', 'telefono' => 'Teléfono', 'direccion' => 'Dirección', 'municipio' => 'Municipio'] as $field => $label): ?>
                                    <div>
                        ...
                        <div style="overflow-x:auto;">
                            <table class="wp-list-table widefat fixed striped">
                                <thead><tr><?php foreach ($preview['headers'] as $h): ?><th><?php echo esc_html($h); ?></th><?php endforeach; ?></tr></thead>
                                <tbody>
                        ...
                        <div style="margin-top:20px;">
                            <button type="submit" class="biodevas-btn biodevas-btn-primary" onclick="return confirm('<?php esc_attr_e('¿Importar todos los socios? Esta acción no se puede deshacer.', 'convoca-members'); ?>');">
                                🚀 <?php echo sprintf(__('Importar %d socios', 'convoca-members'), $preview['total']); ?>
                            </button>
                        </div>
                    </form>
                </div>
    ...
    public function handle_preview(): void
    {
        ...
        if (!$handle) wp_die(__('No se pudo leer el archivo.', 'convoca-members'));

        $headers = fgetcsv($handle, 0, ',', '"', '');
        if (!$headers) { fclose($handle); wp_die(__('CSV vacío o sin cabeceras.', 'convoca-members')); }
        ...
        // Count total rows
        while (fgetcsv($handle, 0, ',', '"', '') !== false) $total++;
        fclose($handle);

        $target_filename = 'bdv_import_' . uniqid() . '.csv';
        copy($filename, sys_get_temp_dir() . '/' . $target_filename);

        set_transient('bdv_csv_preview_' . get_current_user_id(), [
            'headers' => $headers,
            'rows' => $rows,
            'total' => $total,
    ...
    public function handle_template_download(): void
    {
        ...
        $headers = ['nombre', 'email', 'dni', 'plan', 'telefono', 'direccion', 'municipio'];
        $example = ['Juan García López', 'juan@example.com', '12345678Z', 'socio-adulto', '612345678', 'C/ Mayor 123', 'Madrid'];
    ...
    public function handle_import(): void
    {
        ...
        if (!file_exists($filepath)) wp_die(__('Archivo temporal no encontrado. Vuelve a subir el CSV.', 'convoca-members'));

        $map = array_map('sanitize_text_field', $_POST['map'] ?? []);
        ...
        $headers = fgetcsv($handle, 0, ',', '"', '');
        if (!$headers) { fclose($handle); wp_die(__('Error al leer cabeceras.', 'convoca-members')); }
    ...
    private function import_single_row(array $row, array &$errors, int $line, bool $send_welcome, bool $update_existing)
    {
        ...
        if (!is_email($email)) {
            $errors[] = sprintf(__('Fila %d: email inválido (%s).', 'convoca-members'), $line, $email);
            return false;
        }

        // Validate DNI
        if (!empty($dni) && !\Convoca\Core\Utils::validar_dni($dni)) {
            $errors[] = sprintf(__('Fila %d: DNI inválido (%s).', 'convoca-members'), $line, $dni);
            return false;
        }
        ...
        if ($existing_id) {
            if (!$update_existing) {
                if ($dni) {
                    $errors[] = sprintf(__('Fila %d: socio ya existe (email: %s, DNI: %s).', 'convoca-members'), $line, $email, $dni);
                } else {
                    $errors[] = sprintf(__('Fila %d: socio ya existe (email: %s).', 'convoca-members'), $line, $email);
                }
                return false;
            }
            ...
            if (is_wp_error($post_id)) {
                $errors[] = sprintf(__('Fila %d: error al crear (%s).', 'convoca-members'), $line, $post_id->get_error_message());
                return $post_id;
            }
        ...
        $meta_map = ['email', 'dni', 'telefono', 'direccion', 'municipio', 'plan'];
        ...
        if ($send_welcome) {
            do_action('bdv_member_created', $post_id, ['nombre' => $nombre]);
        }

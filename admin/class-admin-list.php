    ...
    public function prepare_items(): void
    {
        ...
        if ($search) {
            ...
            $args['meta_query'] = [
                ...
                ['key' => '_bdv_numero_socio','value' => $search,        'compare' => 'LIKE'],
                ['key' => '_bdv_access_code', 'value' => $search,        'compare' => 'LIKE'],
                ['key' => '_bdv_nombre',      'value' => $search,        'compare' => 'LIKE'],
            ];
        ...
        if (!empty($get_data['voluntarios']) || (isset($get_data['page']) && $get_data['page'] === 'bdv-members-voluntarios')) {
            $args['meta_query'] = array_merge(
                $args['meta_query'] ?? [],
                [['key' => '_bdv_es_voluntario', 'value' => '1']]
            );
        }
        ...
        if (!$search && !$estado_filter && !$tipo_filter && !$plan_filter && !$cuota_filter) {
            set_transient($cache_key, ['items' => $query->posts, 'pagination' => $pagination], 300);
        }
    }

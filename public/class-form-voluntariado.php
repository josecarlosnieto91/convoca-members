...
class Form_Voluntariado
{
    ...
    public function handle_submit(): void
    {
        ...
        if (!\Convoca\Core\Utils::check_rate_limit('bdv_voluntariado_submit', 3, 3600)) {
            wp_send_json_error(['errors' => ['Demasiados intentos. Inténtalo de nuevo en una hora.']], 429);
        }
        ...
        if ($menor) {
            ...
            $tutor_dni = str_replace([' ', '-'], '', $tutor_dni);
            if (empty($tutor_dni)) {
        ...
        if (!is_wp_error($member_post_id)) {
            ...
            $member_meta = [
                ...
                'rgpd_version' => (get_option("bdv_members_settings", [])["rgpd_version"] ?? "1.0"),

...
class PDF_Document {
	...
	private const ERROR_TRANSIENT = 'bdv_pdf_gen_error_';

	public static function init(): void {
		add_action( 'bdv_voluntario_aprobado', array( self::class, 'handle_approval' ) );
		add_filter( 'bdv_voluntario_aprobado_attachments', array( self::class, 'add_pdf_to_email' ), 10, 2 );
	...
	public static function generate_volunteer_agreement( int $user_id ): ?int {
		...
		$existing_id = $wpdb->get_var(
			$wpdb->prepare(
             ...
             AND pm.meta_key = '_bdv_usuario_id' AND pm.meta_value = %d 
		...
		try {
			...
			if ( str_contains( $template_html, '<!-- FIRMA DIGITAL SERÁ AÑADIDA POR LA CLASE BDV_Signature -->' ) ) {

...
class GDPR_Tools
{
    ...
    public static function export_personal_data(string $email_address, int $page = 1): array
    {
        ...
        foreach ($members as $member) {
            ...
            foreach ($pagos as $pago) {
                ...
                $pago_data = [
                    ['name' => 'Concepto', 'value' => get_post_meta($pago->ID, '_bdg_product_desc', true)],
                    ['name' => 'Importe', 'value' => number_format($amount_cents / 100, 2, ',', '.') . ' €'],
                    ['name' => 'Estado', 'value' => get_post_meta($pago->ID, '_bdg_status', true)],
                    ['name' => 'Método', 'value' => get_post_meta($pago->ID, '_bdg_method', true)],
                    ['name' => 'Fecha', 'value' => $pago->post_date],
                ];
            ...
            foreach ($inscripciones as $inscripcion) {
                ...
                $insc_data = [
                    ['name' => 'Actividad', 'value' => $actividad_title],
                    ['name' => 'Estado', 'value' => get_post_meta($inscripcion->ID, '_bde_estado', true)],
                    ['name' => 'Fecha inscripción', 'value' => $inscripcion->post_date],
                ];
            ...
            foreach ($horas as $hora) {
                ...
                $hora_data = [
                    ['name' => 'Fecha', 'value' => get_post_meta($hora->ID, '_bdv_fecha', true)],
                    ['name' => 'Horas', 'value' => get_post_meta($hora->ID, '_bdv_horas', true)],
                    ['name' => 'Proyecto', 'value' => $proyecto_id ? get_the_title($proyecto_id) : ''],
                    ['name' => 'Descripción', 'value' => $hora->post_content],
                    ['name' => 'Estado', 'value' => get_post_meta($hora->ID, '_bdv_estado', true)],
                ];
    ...
    public static function erase_personal_data(string $email_address, int $page = 1): array
    {
        ...
        foreach ($members as $member) {
            ...
            $pii_fields = [
                '_bdv_nombre', '_bdv_apellidos', '_bdv_email', '_bdv_telefono',
                '_bdv_dni', '_bdv_direccion', '_bdv_fecha_nacimiento',
                '_bdv_access_code', '_bdv_consent_ip',
            ];
